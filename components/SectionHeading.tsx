...
export function SectionHeading({ kicker, title, description, align = "left", inverse = false }: SectionHeadingProps) {
  return (
    <div className={`${align === "center" ? "mx-auto max-w-2xl text-center" : "max-w-2xl"}`}>
      <p className={`section-kicker mb-3 ${inverse ? "text-brand-light" : "text-brand-deep"}`}>{kicker}</p>
      <h2 className={`section-title ${inverse ? "text-white" : "text-ink"}`}>{title}</h2>
      {description ? <p className={`mt-5 text-[1.02rem] leading-7 ${inverse ? "text-white/70" : "text-slate"}`}>{description}</p> : null}
