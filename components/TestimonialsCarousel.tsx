...
export function TestimonialsCarousel({ testimonials }: TestimonialsCarouselProps) {
  ...
  return (
    <div
      className="testimonial-stage relative overflow-hidden rounded-card border border-white/90 bg-white/55 shadow-[0_28px_80px_rgba(10,30,54,.11)] ring-1 ring-jidor/5 backdrop-blur-2xl"
      role="region"
    ...
    >
      ...
      <div className="grid gap-10 p-8 sm:p-12 lg:min-h-[430px] lg:grid-cols-[290px_1fr] lg:gap-16 lg:p-16">
        ...
        <div className="flex flex-col">
          <Quote size={42} strokeWidth={1.2} className="text-jidor/25" aria-hidden="true" />
          <p className="mt-6 flex-1 whitespace-pre-line text-lg leading-8 text-ink/90 sm:text-xl sm:leading-9">“{active.quote}”</p>
          <div className="mt-8 border-t border-ink/10 pt-5 text-right">
            <span className="font-mono text-[10px] uppercase tracking-[.16em] text-deep-blue">Avis Google</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 border-t border-white/80 bg-white/30 px-8 py-5 backdrop-blur-md sm:px-12 lg:px-16">
        <div className="flex flex-1 items-center gap-3">
          <div className="flex gap-1.5">
            {testimonials.map((testimonial, i) => (
              <span key={testimonial.id} className={`h-1.5 rounded-pill transition-all duration-300 ${i === index ? "w-6 bg-jidor" : "w-1.5 bg-ink/15"}`} />
            ))}
          </div>
        ...
        <div className="flex shrink-0 gap-2">
          <button type="button" aria-label="Avis précédent" onClick={() => goTo(index - 1)} className="motion-hover flex size-11 cursor-pointer items-center justify-center rounded-full border border-ink/15 bg-white/55 text-ink hover:border-jidor/30 hover:bg-white/90 hover:text-deep-blue">
            <ChevronLeft size={16} strokeWidth={1.8} />
          </button>
          <button type="button" aria-label="Avis suivant" onClick={() => goTo(index + 1)} className="motion-hover flex size-11 cursor-pointer items-center justify-center rounded-full border border-ink/15 bg-white/55 text-ink hover:border-jidor/30 hover:bg-white/90 hover:text-deep-blue">
      ...
      <div className="h-[3px] bg-ink/10">
        {!reduceMotion && (
          <div
            key={index}
            className="testimonial-progress h-full w-full bg-jidor"
